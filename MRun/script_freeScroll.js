window.onload = function(){
	...
	/*document.addEventListener("scroll", function(){
		...
		/*for(var i=0; i<SLIDES; i++){
			if(y>$('.slide[num="' + i + '"]').offset().top){
				cSlide = i;
				i = 10;
			}
		}*/
		//cSlide = Math.floor(y/H)/2;
		//setScroller(cSlide);
		/*if(y<H){
			$("#bg").css({
		...
		else if(pSlide!=cSlide) {
			img_num = Math.floor(Math.random()*IMAGES)+1;//(img_num==IMAGES) ? 1 : img_num+1;  
			$("#bg").css({
				"display":"inline",
	...
	function openTxt(file_name){
	    $.ajax({
            url : file_name,
            dataType: "text",
            success : function (data) {
                //$("pop-content").html(data);
                //document.getElementById("pop-content").innerHTML = data;
                alert(data);
            }
        });
	}
...
//bigger links
//blog on front page
//blog can be just really long page
//schedule in popup